...
class User
{
    ...
    public function registration ($userCode,$userPass,$email,$table){
    ...
    public function checkUserExist($userCode,$email,$table){
        global $dbcon;
...
if ($stmt->rowCount() > 0) {
  ...
  public function login ($userCode,$upass,$table)  {
    global $dbcon;

    $sql = " SELECT * FROM {$table} WHERE usercode = '{$userCode}' AND password = '{$upass}'  ";
...
echo  $reg->checkUserExist('gsmashik',"oleeahmmed@gmail.com","user");
